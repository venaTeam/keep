...
Keep uses **Server-Sent Events (SSE)** to push updates to the UI in real-time. This allows the backend to stream events to the browser over a single HTTP connection.

### Update Flow
1.  **Notification**: After the Worker saves the alert, it sends a notification to the API server (via HTTP POST).
2.  **Broadcast**: The API server broadcasts the event to all connected clients via SSE.
    *   Events: `poll-alerts`, `incident-change`, `poll-presets`, `topology-update`.
3.  **Reaction**: The Browser receives the event.
    *   For `poll-alerts`: The payload contains the new alerts, which are optimistically appended to the UI.
    *   For other events: The UI may re-fetch data or update local state.
...
sequenceDiagram
    ...
    Worker->>API: POST /sse/notify (Internal)
    API->>Browser: Send Event (SSE)
    par Handlers
        Browser->>Browser: Optimistic Update (Append Alert)
    and
        Browser->>API: GET /alerts (Background Sync)
    end
```
...
Finally, we use **Server-Sent Events (SSE)** to notify the frontend in real-time. This makes the UI feel "real-time" without polling.

```python
    # 7. Real-time UI Updates
    with tracer.start_as_current_span("process_event_notify_client"):
        # Notify all connected SSE clients
        await notify_sse_clients(tenant_id, "poll-alerts", alert_data)
        # ...
```
