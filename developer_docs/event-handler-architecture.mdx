...
### 4.1 API Gateway (Ingestion)

The API (**`keep-api`**) is now a lightweight entry point. It validates authentication and then uses an abstract `EventProducer` to offload the raw event.
...
async def receive_event(
    ...
    # Use the abstract event producer
...
The system uses a pluggable messaging layer located in the API core. This allows switching between Redis and Kafka via environment variables.

**Key Files**:
*   **Interfaces**: [`keep/api/core/messaging.py`](file:///Users/yarin/keep/keep/api/core/messaging.py)
*   **Dependency Injection**: [`keep/api/core/dependencies.py`](file:///Users/yarin/keep/keep/api/core/dependencies.py)

```python
# keep/api/core/messaging.py
...
### 4.3 Event Handler Service (Processing)

The dedicated worker service (**`keep-event-handler`**) consumes messages and runs the processing pipeline.

**Key Files**:
*   **Kafka Consumer**: [`keep/event_handler/core/kafka_consumer.py`](file:///Users/yarin/keep/keep/event_handler/core/kafka_consumer.py)
*   **Core Processing Logic**: [`keep/common/event_management/process_event_task.py`](file:///Users/yarin/keep/keep/common/event_management/process_event_task.py)

## 5. Developer Onboarding: Quick Reference
...
| **Abstractions** | Producer Interfaces | [`keep/api/core/messaging.py`](file:///Users/yarin/keep/keep/api/core/messaging.py) |
| **DI** | Dependency Injection | [`keep/api/core/dependencies.py`](file:///Users/yarin/keep/keep/api/core/dependencies.py) |
...
This architecture overhaul transforms Keep into a scalable event processing platform. By decoupling the API from the worker and introducing the `EventProducer`/`EventConsumer` abstraction in the API core, the system supports high-throughput ingestion via Kafka while maintaining backward compatibility with Redis.
