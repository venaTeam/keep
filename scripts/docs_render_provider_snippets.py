    ...
    for kw in field_node.keywords:
        if kw.arg == "metadata" and isinstance(kw.value, ast.Dict):
            ...
            for i, key_node in enumerate(kw.value.keys):
                if isinstance(key_node, ast.Constant) and i < len(kw.value.values):
                    key = key_node.value
        ...
        for provider_property in class_def.body:
            ...
            if (
                ...
                if (
                    ...
                    result["webhook_docs"][provider_property.targets[0].id] = (
                ...
                if docstring:
                    ...
                    if parsed_docstring.params:
                        for param in parsed_docstring.params:
                            result[provider_property.name][param.arg_name] = (
                                param.description
                            )
        ...
        if is_outdated:
            ...
            if not args.validate:
                ...
                fixed_content = re.sub(
                    r"(https://github\.com/keephq/keep/blob/main/examples)\\(workflows)\\",
                    r"\1/\2/",
                    fixed_content,
                )
