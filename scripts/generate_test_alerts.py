    ...
    try:
        with urllib.request.urlopen(req) as response:
            ...
            if 200 <= status_code < 300:
                print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Sent alert {alert['id']}: Success ({status_code})")
            else:
                print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Sent alert {alert['id']}: Failed ({status_code})")
    except urllib.error.HTTPError as e:
        print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Sent alert {alert['id']}: Failed ({e.code}) - {e.reason}")
    except urllib.error.URLError as e:
        print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Error sending alert: {e.reason}")
    except Exception as e:
        print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Error sending alert: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--interval", type=float, default=1.0, help="Interval between alerts in seconds")
    parser.add_argument("--api-key", default="dummy-api-key", help="API Key to include in the request headers")
