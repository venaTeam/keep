...
def test_default_deduplication_rule(db_session, client, test_app):
    ...
    for dedup_rule in deduplication_rules:
        ...
        # check that the mock/prometheus deduplication rule is working
        else:
...
def test_deduplication_sanity(db_session, client, test_app):
    ...
    assert len(deduplication_rules) == 2  # default + mock
...
def test_deduplication_sanity_2(db_session, client, test_app):
    ...
    alert2 = alert1
    # mock deduplicated by fingerprint
...
def test_deduplication_sanity_3(db_session, client, test_app):
    ...
    fingerprints = set()
    for alert in alerts:
        # lets make it not deduplicated by randomizing the fingerprint
        while alert["fingerprint"] in fingerprints:
...
def test_custom_deduplication_rule(db_session, client, test_app):
    ...
    custom_rule = {
        ...
        "fingerprint_fields": ["name", "description"],
...
def test_custom_deduplication_rule_behaviour(db_session, client, test_app):
    ...
    for _ in range(2):
        # the default rule should deduplicate the alert by fingerprint so let's randomize it -
...
def test_custom_deduplication_rule_2(db_session, client, test_app):
    ...
    custom_rule = {
        ...
        "fingerprint_fields": [
            "name",
            "description",
        ],  # title in mock mapped to name in keep
        "full_deduplication": False,
    ...
    client.post(
        f"/alerts/event/prometheus?provider_id={mock_provider_id}",
        json=alert1,
        headers={"x-api-key": "some-api-key"},
    )
    alert1["labels"]["alertname"] = "Different title"
    client.post(
        f"/alerts/event/prometheus?provider_id={mock_provider_id}",
        json=alert1,
        headers={"x-api-key": "some-api-key"},
...
def test_update_deduplication_rule(db_session, client, test_app):
    ...
    for dedup_rule in deduplication_rules:
        if dedup_rule.get("id") == rule_id:
            ...
            assert dedup_rule.get("fingerprint_fields") == ["name"]
...
def test_partial_deduplication(db_session, client, test_app):
    # insert a mock alert with the same incident_id, group and title and make sure that the mock default deduplication rule is working
    provider = ProvidersFactory.get_provider_class("prometheus")
...
def test_ingesting_alert_without_fingerprint_fields(db_session, client, test_app):
    # insert a mock alert without the required fingerprint fields and make sure that it is not deduplicated
    provider = ProvidersFactory.get_provider_class("prometheus")
    alert = provider.simulate_alert()
...
def test_deduplication_fields(db_session, client, test_app):
    # insert a mock alert with the same incident_id and make sure that the mock default deduplication rule is working
    provider = ProvidersFactory.get_provider_class("prometheus")
    base_alert = provider.simulate_alert()
