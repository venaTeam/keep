...
def test_dismissal_expiry_bug_search_filters(db_session):
    ...
    with freeze_time(dismiss_time):
        ...
        dismissed_alerts = SearchEngine(tenant_id=tenant_id).search_alerts(
            search_query_dismissed
        )
        assert len(dismissed_alerts) == 1, (
            "Alert should be dismissed during dismissal period"
        )
    ...
    with freeze_time(after_expiry_time):
        ...
        non_dismissed_alerts_before = SearchEngine(tenant_id=tenant_id).search_alerts(
            search_query_not_dismissed
        )
        ...
        non_dismissed_alerts_after = SearchEngine(tenant_id=tenant_id).search_alerts(
            search_query_not_dismissed
        )
...
def test_dismissal_expiry_bug_sidebar_filter(db_session):
    ...
    with freeze_time(dismiss_time):
        ...
        current_not_dismissed = SearchEngine(tenant_id=tenant_id).search_alerts(
            not_dismissed_query
        )
    ...
    with freeze_time(after_expiry_time):
        ...
        current_not_dismissed_before = SearchEngine(tenant_id=tenant_id).search_alerts(
            not_dismissed_query
        )
        ...
        returned_fingerprints_before = {
            alert.fingerprint for alert in current_not_dismissed_before
        }
        ...
        current_not_dismissed_after = SearchEngine(tenant_id=tenant_id).search_alerts(
            not_dismissed_query
        )
        ...
        returned_fingerprints_after = {
            alert.fingerprint for alert in current_not_dismissed_after
        }
        ...
        temporary_alert = next(
            alert
            for alert in current_not_dismissed_after
            if alert.fingerprint == "temporary-fp"
        )
        assert temporary_alert.dismissed == False, (
            "Temporary alert should now be un-dismissed"
        )
        assert temporary_alert.dismissUntil is None, (
            "Temporary alert dismissUntil should be cleared"
        )
...
def test_dismissal_expiry_bug_with_cel_filter(db_session):
    ...
    with freeze_time(past_expiry_time):
        ...
        for cel_expr in cel_expressions:
            ...
            results_before = SearchEngine(tenant_id=tenant_id).search_alerts(
                search_query
            )
        ...
        for cel_expr in cel_expressions:
            ...
            results_after = SearchEngine(tenant_id=tenant_id).search_alerts(
                search_query
            )
...
def test_dismissal_forever_works_correctly(db_session):
    ...
    with freeze_time(current_time):
        ...
        results = SearchEngine(tenant_id=tenant_id).search_alerts(not_dismissed_query)
        assert len(results) == 0, (
            "Alert with dismissUntil='forever' should not appear in non-dismissed filter"
        )
    ...
    with freeze_time(future_time):
        ...
        results = SearchEngine(tenant_id=tenant_id).search_alerts(not_dismissed_query)
        assert len(results) == 0, (
            "Alert with dismissUntil='forever' should remain dismissed even after watcher runs"
        )
...
def test_dismissal_expiry_bug_fixed_with_watcher(db_session):
    ...
    with freeze_time(after_expiry_time):
        ...
        assert len(non_dismissed_alerts_before) == 0, (
            "Before watcher: bug still exists, alert not returned"
        )
        ...
        assert len(non_dismissed_alerts_after) == 1, (
            "After watcher: Alert should appear in non-dismissed filter"
        )
        assert non_dismissed_alerts_after[0].dismissed == False
        assert non_dismissed_alerts_after[0].dismissUntil is None
        ...
        assert (
            not hasattr(non_dismissed_alerts_after[0], "disposable_dismissed")
            or getattr(non_dismissed_alerts_after[0], "disposable_dismissed", None)
            is None
        )
...
def test_dismissal_expiry_boolean_comparison_fix(db_session):
    ...
    expired_dismissals = DismissalExpiryBl.get_alerts_with_expired_dismissals(
        db_session
    )
    ...
    for test_case in test_cases:
        ...
        test_found = next(
            e
            for e in expired_dismissals
            if e.alert_fingerprint == test_case["fingerprint"]
        )
...
def test_dismissal_expiry_status_and_disposable_fields_cleanup(db_session):
    ...
    with freeze_time(after_expiry_time):
        ...
        assert enrichment.enrichments.get("dismissed") == False, (
            "dismissed should be False after watcher"
        )
        assert enrichment.enrichments.get("dismissUntil") is None, (
            "dismissUntil should be None after watcher"
        )

        # Test 2: Status should be removed from enrichments (let AlertDto use original alert status)
        assert (
            "status" not in enrichment.enrichments
            or enrichment.enrichments.get("status") != "suppressed"
        ), (
        ...
        print(
            f"Final AlertDto status: {actual_status} (should be original alert status, not 'suppressed')"
        )
...
def test_github_issue_5047_cel_filters_dismisseduntil_bug_fixed(db_session):
    ...
    with freeze_time(dismiss_time):
        ...
        assert len(dismissed_alerts) == 1, (
            "Alert should be dismissed during active dismissal period"
        )
        assert dismissed_alerts[0].dismissed == True
        assert dismissed_alerts[0].dismissUntil == dismiss_until_str
    ...
    with freeze_time(after_expiry_time):
        ...
        sidebar_results_before = SearchEngine(tenant_id=tenant_id).search_alerts(
            sidebar_not_dismissed_query
        )
        cel_results_before = SearchEngine(tenant_id=tenant_id).search_alerts(
            cel_dismissed_false_query
        )

        assert len(sidebar_results_before) == 0, (
            "Bug reproduction: Sidebar filter should return 0 (bug exists)"
        )
        assert len(cel_results_before) == 0, (
            "Bug reproduction: CEL filter should return 0 (bug exists)"
        )
        ...
        sidebar_results_after = SearchEngine(tenant_id=tenant_id).search_alerts(
            sidebar_not_dismissed_query
        )
        cel_results_after = SearchEngine(tenant_id=tenant_id).search_alerts(
            cel_dismissed_false_query
        )

        # ✅ GITHUB ISSUE #5047 SOLVED! ✅
        assert len(sidebar_results_after) == 1, (
            "FIXED: Sidebar 'Not dismissed' filter now works after watcher"
        )
        assert len(cel_results_after) == 1, (
            "FIXED: CEL 'dismissed == false' filter now works after watcher"
        )
        ...
        assert sidebar_alert.dismissed == False, (
            "Alert should be un-dismissed after expiry"
        )
        ...
        assert (
            not hasattr(sidebar_alert, "disposable_dismissed")
            or getattr(sidebar_alert, "disposable_dismissed", None) is None
        )
        assert (
            not hasattr(sidebar_alert, "disposable_note")
            or getattr(sidebar_alert, "disposable_note", None) is None
        )
        ...
        for cel_expr in additional_cel_filters:
            ...
            assert len(results) == 1, (
                f"Additional CEL '{cel_expr}' should also work after fix"
            )
            assert results[0].dismissed == False
...
def test_dismissal_expiry_bug_search_filters_FIXED_with_watcher(db_session):
    ...
    with freeze_time(after_expiry_time):
        ...
        assert len(non_dismissed_alerts_before) == 0, (
            "Before watcher: bug exists, alert not returned"
        )
        ...
        assert len(non_dismissed_alerts_after) == 1, (
            "FIXED: Alert appears in non-dismissed filter after watcher"
        )
        assert non_dismissed_alerts_after[0].dismissed == False
        assert non_dismissed_alerts_after[0].dismissUntil is None
