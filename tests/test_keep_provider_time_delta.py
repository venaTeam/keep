...
def test_keep_provider_time_delta_filtering_bug(db_session):
    ...
    provider = KeepProvider(
        context_manager=context_manager, provider_id="test-keep", config=provider_config
    )
    ...
    for detail in alert_details:
        ...
        timestamp = datetime.datetime.fromisoformat(
            detail["lastReceived"].replace("Z", "+00:00")
        )
    ...
    assert len(results) == 1, (
        f"Expected 1 alert within time_delta, but got {len(results)}"
    )
...
def test_keep_provider_time_delta_filtering_version_1(db_session):
    ...
    assert len(filtered_alerts) == 1, (
        f"Expected 1 alert within time_delta, but got {len(filtered_alerts)}"
    )
